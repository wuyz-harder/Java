...
/**
 * 一个一个节点的处理，在complatework()里面会根据sibing重置（也就是递归的归）
 * @params unitOfWork 当前需要处理的节点
 */
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  // 尝试完成当前节点得工作，然后完成下一个兄弟节点的工作。
  // 如果没有兄弟就跳到父节点
  // Attempt to complete the current unit of work, 
  ...
  do {
    ...
    //此fiber的当前冲洗状态为备用状态。理想的
    //任何事情都不应该依赖于此，但依赖于此意味着我们不需要
    //需要有关正在进行的工作的附加字段。就是理想情况下没有变化的话可以直接执行
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      if (
        ...
        // 如果这fiber有副作用，我们会在children的副作用之后添加它。
        // 如果需要，我们可以通过对效果列表进行多次遍历，
        // 更早地执行某些副作用。
        // 我们不想在我们自己的列表中安排我们自己的副作用，
        // 因为如果最终重用了孩子，我们会将这种影响安排到自己身上，因为我们已经结束了。
        ...
        // committed.
        // flags > PerformedWork代表当前这个fiber节点是有副作用的
        // 需要将这个fiber节点加入到父级fiber的effectList链表中。
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  if (firstEffect !== null) {
    ...
    // 提交阶段分为几个子阶段。
    // 我们为每个阶段单独传递效果列表
    // ：所有mutation effects先于所有layout effects，依此类推

    // The first phase a "before mutation" phase. We use this phase to read the
    // state of the host tree right before we mutate it. This is where
    // getSnapshotBeforeUpdate is called.
    // 第一阶段是“before mutation”阶段。
    // 我们使用这个阶段在变异宿主树之前读取它的状态。
    ...
    // 执行下一个阶段是layout（布局）阶段，
    // 我们调用在宿主树发生变异后读取宿主树的效果。
    // 这方面的惯用用例是布局，但由于遗留原因，
    // 类组件生命周期也在这里启动
    nextEffect = firstEffect;
    do {
...
// 总结： before-mutation阶段之前，会调度useEffect，
// before-mutation阶段，会遍历effectList，依此执行处理dom节点渲染/删除后的foucs, blue等逻辑，
// 调用getSnapshotBeforeUpdate生命周期钩子，
// layout阶段之后，会做一系列事情，包括给全局变量赋值。
// useEffect异步调度的逻辑就是
// 在before-mutation阶段之前调度flushPassiveEffects，
// 然乎lyaoyt阶段之后赋值rootWithPendingPassiveEffects ,
// 最后等待scheduleCallback触发回调函数，执行flushPassiveEffects。
